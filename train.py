...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if opt.cos_lr:
        ...
        lf = lambda x: (1 - x / (epochs - hyp['warmup_epochs'])) * (1.0 - hyp['lrf']) + hyp['lrf']  # linear
    if not batchwise: 
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= nw:
                ...
                for j, x in enumerate(optimizer.param_groups):
                    ...
                    x['lr'] = np.interp(ni, xi, [hyp['warmup_bias_lr'] if j == 0 else 0.0, x['initial_lr']])
                    if 'momentum' in x:
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not (check_wandb_resume(opt) or opt.evolve):  # resume from specified or most recent last.pt
        ckpt = Path(check_file(opt.resume) if isinstance(opt.resume, str) else get_latest_run())
        ...
        # this may break if opt is changed by a commit and user tries to resume:
        opt = argparse.Namespace(**d)  # replace
        # if so, try this:
        #resume_opt = argparse.Namespace(**d)
